{self, ...}: let
  name = "t14s";
in {
  flake.modules.nixos."${name}DolbySound" = {...}: {
    home-manager.users.walt.imports = [
      self.modules.homeManager."${name}DolbySound"
    ];
  };

  flake.modules.homeManager."${name}DolbySound" = {...}: {
    services.easyeffects = {
      enable = true;
      preset = "${name}";
      extraPresets = {
        # t14s gen2 preset found here:
        # github:sebastian-de/easyeffects-thinkpad-unsuck
        ${name} = {
          "output" = {
            "bass_enhancer#0" = {
              "amount" = 22.0;
